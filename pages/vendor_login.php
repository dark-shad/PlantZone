<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        $query=mysqli_query($dbconn,"SELECT * FROM `vendor` WHERE username='$user' AND password='$pass'");
        $res=mysqli_fetch_array($query);
        $id=$res['user_id'];

        if (mysqli_num_rows($query)<1){
            $_SESSION['msg']="Login Failed, vendor not found!";
            ...
            $_SESSION['id']=$id;
            $remarks="(Vendor) has logged in the system at ";  
